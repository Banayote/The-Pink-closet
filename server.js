...
// Route for adding a new product (with image upload) - restricted to user role
app.post('/api/products', authenticateToken, upload.single('image'), async (req, res) => {
  try {
    ...
    const newProduct = new Product({
      ...
      description,
      category,
      userId: req.user.id // Link product to the user
    });
...
app.get('/add-product.html', authenticateToken, (req, res) => {
  res.send(`
    <!DOCTYPE html>
    <html lang="en">
    <head>
    ...
    <body>
        ...
        <form id="add-product-form" action="/api/products" method="POST" enctype="multipart/form-data">
        ...
        <button onclick="location.href='/profile/${req.user.profileLink}'">Back to Profile</button>
    </body>
    </html>
  `);
});
